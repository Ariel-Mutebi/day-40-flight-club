...
class Announcer:
        ...
        def announce(self, mailing_list, historical_price, price, departure_airport, destination_airport, outbound_date, inbound_date, number_of_stops):
                with open("email_template.txt") as email_template:
                        ...
                        email_body.replace("[DEPARTURE_AIRPORT]", departure_airport)
                        email_body.replace("[DESTINATION_AIRPORT]", destination_airport)
                        email_body.replace("[HISTORICAL_PRICE]", str(historical_price))
                        email_body.replace("[PRICE]", str(price))
                        email_body.replace("[OUTBOUND_DATE]", outbound_date)
                        email_body.replace("[INBOUND_DATE]", inbound_date)
                        email_body.replace("[NUMBER_OF_STOPS]", str(number_of_stops))
